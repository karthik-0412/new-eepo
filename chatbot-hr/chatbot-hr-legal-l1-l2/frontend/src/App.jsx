    ...
    <div className="app-root dark">
      ...
      <main className="main">
        ...
        {view === 'chat' ? (
          ...
          <FilesPanel selectedDomain={domain} />
        )}
      </main>
    </div>
  );
...
function FilesPanel({ selectedDomain = 'hr' }) {
  const [files, setFiles] = useState([]);
  const [loading, setLoading] = useState(false);
  const [fileDomain, setFileDomain] = useState(selectedDomain === 'auto' ? 'hr' : selectedDomain);

  // Reload when selectedDomain prop changes
  useEffect(() => {
    const domainToUse = selectedDomain === 'auto' ? 'hr' : selectedDomain;
    setFileDomain(domainToUse);
  }, [selectedDomain]);

  // Load files whenever fileDomain changes
  useEffect(() => {
    loadFiles();
  }, [fileDomain]);
  ...
  async function loadFiles() {
    setLoading(true);
    try {
      const res = await fetch(`/api/files/list?domain=${encodeURIComponent(fileDomain)}`);
      const data = await res.json();
      setFiles(data);
    } catch (e) {
  ...
  return (
    <div style={{ padding: 20 }}>
      <div style={{ marginBottom: 12, display: 'flex', gap: 12, alignItems: 'center' }}>
          <select value={fileDomain} onChange={(e) => setFileDomain(e.target.value)} style={{ padding: 8, borderRadius: 8 }}>
